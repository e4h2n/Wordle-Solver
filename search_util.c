...
int score_word(char *word, int *letter_scores) {
  // TODO(you): implement this function!
  int scratch_scores[26]; // don't want to mutate letter_scores
  memcpy(scratch_scores, letter_scores, 26*sizeof(int));

  int idx;
  int total = 0;
  for(int i = 0; i < 5; i++){
    idx = word[i]-'a';
    total += scratch_scores[idx];
    scratch_scores[idx] = 0;
  }
  return total;
}
...
size_t filter_vocabulary_yellow(char letter, int position, char **vocabulary,
  ...
  for(size_t i = 0; i < num_words; i++){
    ...
    int del = 1;
    for(int j = 0; j < 5; j++){
      int match = (vocabulary[i][j]==letter);
      if (match && position == j){
        del = 1;
        break;
      } else if (match && position != j){
        del = 0;
      }
    }
    if (del){
